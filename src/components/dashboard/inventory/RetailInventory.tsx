...
export function RetailInventory({ products, categories, summary, loading, onRefresh }: RetailInventoryProps) {
  ...
  const handleOpenModal = (product?: Product) => {
    if (product) {
      ...
      setFormData({
        ...
        trackByRound: product.track_by_round || false
      });
      setImagePreview(product.image_url || null);
    } else {
      setEditingProduct(null);
      setFormData({ name: '', price: '', cost: '', stock: '', categoryId: '', minStockLevel: '', trackByRound: false });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      data.append('track_by_round', formData.trackByRound.toString());
                      ...
                      <td className="px-6 py-4 text-center">
                        <span className={cn(
                          ...
                          "bg-rose-50 text-rose-700"
                           ...
                           {product.track_by_round ? `${product.stock} KG` : `${product.stock} units`} in stock
                        </span>
                      </td>
             ...
             <div className="flex-1 overflow-y-auto p-6">
                <form id="productForm" onSubmit={handleSubmit} className="space-y-5">
                   ...
                   <div className="space-y-4">
                      ...
                      <div className="grid grid-cols-2 gap-4">
                          <div>
                             <label className="block text-sm font-medium text-slate-700 mb-1">
                                Stock Quantity {formData.trackByRound ? '(KG)' : '(Units)'}
                             </label>
                             <input 
                                type="number"
                          ...
                          <div>
                             <label className="block text-sm font-medium text-slate-700 mb-1">
                                Min. Stock Level {formData.trackByRound ? '(KG)' : '(Units)'}
                             </label>
                             <input 
                                type="number"
                      ...
                      {business?.type?.includes('LPG_STATION') && (
                        <div className="bg-teal-50 p-4 rounded-xl border border-teal-100 flex items-center justify-between">
                            <div>
                                <p className="text-sm font-bold text-teal-900">Track by Round (Bulk)</p>
                                <p className="text-[10px] text-teal-600 font-medium">Enable for LPG/Fuel to track large supply batches.</p>
                            </div>
                             <button
                                 type="button"
                                 onClick={() => {
                                    setFormData(prev => ({ 
                                        ...prev, 
                                        trackByRound: !prev.trackByRound
                                    }));
                                 }}
                                 className={cn(
                                     "w-12 h-6 rounded-full transition-colors relative",
                                     formData.trackByRound ? "bg-teal-600" : "bg-slate-300"
                                 )}
                             >
                                 <div className={cn(
                                     "absolute top-1 w-4 h-4 rounded-full bg-white transition-all",
                                     formData.trackByRound ? "left-7" : "left-1"
                                 )} />
                             </button>
                        </div>
                      )}
